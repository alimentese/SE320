...
public class BEnemy : MonoBehaviour
{
    ...
    void Update(){
        ...
        float distToPlayer = Vector2.Distance(transform.position, player);
    ...
    void ChasePlayer(){
         if((transform.position.x < player..x + 0.1 )){
             //right move
         ...
         else if (transform.position.x > player.x - 0.4 ){
             //left move
             EnemyRb.velocity = new Vector2(-EnemySpeed,EnemyRb.velocity.y);
             //transform.localScale = new Vector2(-1,1);
             direction();
    ...
    void direction(){
     ...
     void jumping(){
                ...
                if(player.transform.position.y - transform.position.y > 2 && isGrounded  && ontrigger == true ){
                    //jump
