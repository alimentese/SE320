...
public class EnemyScript : MonoBehaviour
{
    ...
    private MushroomTrigger Mushroomtrigger;
    Animator EnemyAnim;
    Rigidbody2D  EnemyRb;
    ...
    void Start(){
        ...
        Mushroomtrigger = TriggerArea.GetComponent<MushroomTrigger>();
    ...
    void Update(){
        ...
        isGrounded = Physics2D.OverlapBox(new Vector2(gameObject.transform.position.x, gameObject.transform.position.y - 1.0f),new Vector2(0.9f, 0.4f),0f, groundMask);
         ...
         if(distToPlayer < AgroRange){
             if(Mushroomtrigger.trigger == true && isGrounded){
                 Attack();
                 }
                 else{
                    ChasePlayer();
